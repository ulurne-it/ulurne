...
import { Bell, Search, User, Sparkles, Plus } from 'lucide-react';
      ...
      >
        <div className="h-full flex items-center justify-between bg-[#0a0a0f]/60 backdrop-blur-2xl border-b border-white/5 px-6 shadow-xl">
          ...
          <div className="flex items-center gap-4">
            <Link href="/app" className="flex lg:hidden items-center gap-2 cursor-pointer">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="group flex items-center gap-2 px-4 py-2 rounded-xl bg-primary/10 border border-primary/20 hover:bg-primary transition-all shadow-xl shadow-primary/5 cursor-pointer"
            >
              <Sparkles className="w-3.5 h-3.5 text-primary group-hover:text-white transition-colors" />
              <span className="text-[10px] font-black uppercase tracking-widest text-primary group-hover:text-white hidden lg:block">Enter Studio</span>
            </button>

            <div className="w-px h-6 bg-white/5 mx-2 hidden lg:block" />

            <button className="lg:hidden p-2 rounded-xl bg-white/5 hover:bg-white/10 transition-colors cursor-pointer">
            ...
            <div className="relative">
              <button
                ...
                className={`p-2 rounded-xl transition-all relative group cursor-pointer ${isNotificationsOpen ? 'bg-primary/20 text-primary' : 'bg-white/5 hover:bg-white/10 text-muted hover:text-white'}`}
              >
                <Bell className="w-4 h-4 transition-colors" />
                {unreadCount > 0 && (
                  <span className="absolute top-2 right-2 w-1.5 h-1.5 bg-primary rounded-full animate-pulse shadow-[0_0_8px_rgba(var(--primary-rgb),0.8)]" />
                )}
              </button>
            ...
            {/* User Profile */}
            <Link href="/profile" className="flex items-center gap-2.5 hover:bg-white/5 p-1 rounded-xl transition-all group cursor-pointer">
              <UserAvatar
              ...
              <div className="hidden md:block">
                <p className="text-[9px] font-black uppercase tracking-widest leading-none mb-0.5 group-hover:text-primary transition-colors">
